...
function BookingAll() {
    ...
    // ];
    const Column= [
        {title:'Mã booking', dataIndex:'id', render:(text:any)=><span>{`B00${text}`}</span>},
            ...
            <Row style={{display:"flex"}}>
                <Col span={6} style={{display:"flex", justifyContent:"center", alignItems:"center"}}>
                    <Space direction="vertical" size={"large"} style={{textAlign:"center"}} >
                        <span style={{fontWeight:500, fontSize:20}}>Tổng doanh thu</span >   
                        <h1 style={{fontWeight:500, fontSize:40}}>1.250.000</h1>
                        <span style={{fontWeight:500, fontSize:20}}>Booking chi tiết</span>
                    </Space>
                </Col>
                <Col span={12} style={{width:"100%"}}>
                    <div style={{ width: "100%" }}>
                        <ResponsiveContainer width={"100%"} height={200}>
                            <LineChart data={dataDoanhthu} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                                <CartesianGrid strokeDasharray="3 3" />
                                <XAxis dataKey="month" />
                                <YAxis />
                                <Tooltip />
                                <Line type="monotone" dataKey="bookings" stroke="#1890ff" strokeWidth={2} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
