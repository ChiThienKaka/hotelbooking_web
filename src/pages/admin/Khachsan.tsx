...
function Khachsan() {
    ...
    const Column= [
        ...
        {title:'Tên khách sạn', dataIndex:'name', render:(text:any)=><span>{`${text}`}</span>},
        {title:'Thành phố', dataIndex:'city', render:(text:any)=><span>{`${text}`}</span>},
        {title:'Quốc gia', dataIndex:'country', render:(text:any)=><span>{`${text}`}</span>},
        {title:'Xếp hạng', dataIndex:'rate', render:(text:any)=><Rate disabled value={text} />}, 
