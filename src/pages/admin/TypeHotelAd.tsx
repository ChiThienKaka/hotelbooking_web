...
function TypeHotelAd() {
    ...
    const Column= [
        ...
        {title:'Loại khách sạn', dataIndex:'name', render:(text:any)=><span>{`${text}`}</span>},
        {title:'Mô tả chi tiết', dataIndex:'description', render:(text:any)=><span>{`${text}`}</span>},
    ...
    return ( 
        <div style={{padding:20}}>
            ...
            <Divider />
            {
                dataHotel && <Table columns={Column}  dataSource={dataHotel} pagination={{pageSize:5}} />
            }
        </div>
     );
}
